-- Last-N-games equivalent of player_stats_for_period. Unlike the period
-- functions, this is NOT zero-sum: each player's own most recent p_n games
-- are selected independently, so different players' games can come from
-- different dates.
CREATE FUNCTION public.player_stats_for_last_n(p_n integer)
RETURNS TABLE(
...
AS $function$
  WITH ranked AS (
    SELECT
      r.player_id, r.game_id, r.rank, r.final_score, g.played_at,
      row_number() OVER (PARTITION BY r.player_id ORDER BY g.played_at DESC, r.game_id DESC) AS rn
    FROM results r
    JOIN games g ON g.game_id = r.game_id
  ),
  filtered AS (
    SELECT player_id, game_id, rank, final_score, played_at
    FROM ranked
    WHERE rn <= p_n
  ),
  base AS (
