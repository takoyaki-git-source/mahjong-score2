...
- **配色トークン**(`src/app/globals.css`の`:root`): `--background`(生成り/暗い緑黒), `--foreground`, `--foreground-soft`(補助テキスト), `--accent`(中の赤、警告/ワースト/CTA), `--accent-2`(發の緑、ベスト/成功), `--gold`/`--silver`/`--bronze`(金銀銅、`TileBadge`の1〜3位に対応), `--line`(罫線), `--surface`(カード背景)。Tailwindの`@theme inline`で`bg-accent`等のユーティリティにマッピング
- **テーマ選択**(`ThemeToggle`、両ヘッダーに設置): 端末に合わせる(デフォルト)/ライト/ダーク/發(緑基調)/中(赤基調)の5択。`<html data-theme="...">`属性で切り替え、値は`localStorage`(キー`theme`)に保存。属性なし=「端末に合わせる」で`prefers-color-scheme`に追従。`globals.css`のダークmedia queryは`:root:not([data-theme])`にスコープし、明示選択時はOS設定より優先されるようにしている。`layout.tsx`の`<head>`にブロッキングな`<script>`(`localStorage`を読んで`data-theme`を同期的にセット)を入れ、hydration前の一瞬だけ別テーマの色が見えてしまうフラッシュを防止(`<html>`には`suppressHydrationWarning`が必要)。⚠️ **`--accent`(中/赤)と`--accent-2`(發/緑)は全テーマで意味(ワースト/ベスト)を固定**していて、テーマごとに変えているのは`--background`/`--surface`/`--foreground`/`--line`などの中立色のみ(ランキングのハイライト色がテーマによって意味を持たなくなるのを防ぐため)。發/中テーマは`color-scheme: light`(ダークではなく生成りを寄せた配色のバリエーション)
...
- `src/app/page.tsx`(トップページ、`/`): 成績一覧。`PeriodSelector`で期間指定(全期間/直近1年/今年/年単位ボタン`available_years()`/カスタム。直近3ヶ月はユーザー希望で削除)し、`player_stats_for_period` RPCの結果を`Leaderboard`コンポーネントで表示。下部に`SeatStatsTable`(座席別成績、`seat_stats_for_period` RPC)も表示。プレイヤーを問わず開始時の座席(東家/南家/西家/北家)ごとに全員分を合算し、起家が統計的に有利かを分析できるようにしたもの。直近N半荘モードは「プレイヤーごとの直近N半荘」という概念と座席横断集計の相性が悪いため対応せず、常に全期間(`p_start`/`p_end`とも`null`)を渡す(画面にもその旨を注記)
...
- `src/app/players/page.tsx`: プレイヤー一覧(公開ページ、`/players`)。統計テーブルを経由せず名前だけを`Intl.Collator('ja')`近似の五十音順で並べた軽い入り口ページ(成績一覧の名前クリックだけが個人ページへの導線だったため追加)。`SiteHeader`のナビにも追加
- `src/app/changelog/page.tsx` / `docs/changelog.md`: 更新履歴ページ。全コミットログではなく**使い勝手に関わる更新だけを手動で厳選**して`docs/changelog.md`に追記していく方式(コミットメッセージにはDB内部のバグ修正など友人向けには不要な情報も混ざるため、自動生成ではなく都度このファイルを手動更新する)。バージョンは日付ベース(`v2026.09.20`のような出荷日単位)。`src/lib/changelog.ts`の`latestVersion()`が`docs/changelog.md`先頭の`## vX.X.X`見出しをパースして返す(バージョン番号を別ファイルに二重管理しないため)。`src/components/Footer.tsx`(`layout.tsx`で全ページ共通表示)にバージョン+`/changelog`へのリンクを表示。`/rules`ページと共通のMarkdown→JSXマッピング(`src/components/markdownComponents.tsx`の`docsMarkdownComponents`)を使う
...
- **`submit_game_points(p_played_at, p_rule_id, p_player1..4, p_points1..4, p_seat1..4, p_tobi_target, p_tobi_by, p_yakuman) → text`**: `submit_game`と対になる、ポイント直接入力用のRPC。
  ...
  - ⚠️ **2026-08-10分の`seat_order`は信頼できないため後日NULLに戻した**: `GameForm`は登録後にプレイヤー・座席を保持したまま点数だけリセットする設計(同じ顔ぶれで何半荘も続けて入力する運用のため)だが、これは裏を返すと座席を毎半荘手動で選び直さないと同じ座席のまま登録され続けることを意味する。2026-08-10分の10半荘は全て4人とも座席が固定(実際の麻雀では親が半荘ごとに変わり得ないパターン)だったため誤入力と判断し、自風別成績が偏らないよう`seat_order`のみNULLに戻した(`rank`/`final_score`は変更なし)。他の日(2026-08-15、2026-09-19)は半荘ごとに座席が変化しており正常
