...
✅ **対応済み**:
- `player_base_stats`のトビ判定(`final_score <= -50`の閾値ベース)は意図的な設計と確認。`tobi_by_player_id`は`submit_game`呼び出し時の任意引数で、入力し忘れるとNULLのままになる。過去データは`raw_score`がNULLで`tobi_by_player_id`もほぼ未設定なので、閾値ベースは実質唯一の実用的な判定手段。このままでよい
...
過去の成績はGoogleスプレッドシートで管理していた。既に2016-11-05〜2026-04-30分の857半荘はSupabaseに取り込み済み。**最終対局日(2026-04-30)以降、直近数ヶ月分がまだ未取り込みの可能性がある**(要確認)。
